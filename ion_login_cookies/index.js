...
app.use(cookieSession({
  name: 'session',
  keys: ['youwontbeabletoguessthiskeyhahaha1', 'youwontbeabletoguessthiskeyhahaha2']
}));
...
app.get('/', function (req, res, next) {
    ...
    if (typeof req.session.token != 'undefined') {
        ...
        request.get( {url:my_ion_request}, function (e, r, body) {
            ...
            mid = res_object['middle_name'];
            last = res_object['last_name'];


            var output_string = "";
            output_string += "<!doctype html>\n";
            output_string += "<html><head></head><body>\n";
            output_string += "<p>Hello, "+user_name+" "+mid+" "+last+"!</p>\n";
            // output_string += "</body></html>";
            
            output_string += "<img src="+res_object['picture'] +" alt=\'profile\'>";
            
            if(req.session.uservisits)
                var uservisits = req.session.uservisits;
            else
                var uservisits = 0;
            uservisits++;
            req.session.uservisits = uservisits;
            
            
            output_string += "<p>You\'ve visited this page " + uservisits + " times.</p>\n";
            visitorCount++;
            output_string += "<p>You are global visitor number " + visitorCount + ".</p>\n";
            
            output_string += "</body></html>";
            // render_dictionary = {
            //     name : output_string,
            //     pfp : pic,
            //     globalvisits : visitorCount
            // };

            res.send(output_string);
            // res.render('ion', render_dictionary );
        });
